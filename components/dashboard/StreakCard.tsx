        ...
        <div>
          ...
          <div className="mt-1 flex items-center gap-1.5">
            {/* C.6: fuego en vez de un número pelado, opacidad ~ largo de la racha.
                C.5: pulsa una vez al marcar (300ms). */}
            <span
              aria-hidden="true"
              className={justMarked ? "animate-fire-pulse" : ""}
              style={{ opacity: display.current === 0 ? 0.25 : Math.min(1, 0.35 + display.current / 30) }}
            >
              🔥
            </span>
            <CountUp value={display.current} className="font-mono text-3xl font-bold text-text tabular-nums" />
          </div>
        </div>
        <span
          className={
